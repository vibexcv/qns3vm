...
class QN_S3VM_Dense:
    ...
    def __initializeMatrices(self):
        if self.__matrices_initialized == False:
            ...
            for l in self.__L_l:
                x.append(l)
            self.__YL = np.asmatrix(x, dtype=np.float64)
            ...
            self.__Xreg = (np.asmatrix(self.__X)[self.__regressors_indices,:].tolist())
            ...
            self.__KNR = cp.deepcopy(block([[self.__KLR], [self.__KUR]]))
            ...
            self.__X_u_subset = (np.asmatrix(self.__X_u)[subset_unlabled_indices,:].tolist())
...
class QN_S3VM_Sparse:
    ...
    def __init__(self, X_l, L_l, X_u, random_generator, ** kw):
        ...
        for l in L_l:
            x.append(int(l))
        self.__YL = np.asmatrix(x, dtype=np.float64).A
        self.__YL = self.__YL.transpose()
        self.__setParameters( ** kw)
        self.__kw = kw
        self.X_l = X_l.tocsr()
...
class LinearKernel():
    ...
    def computeKernelMatrix(self, data1, data2, symmetric=False):
        ...
        self._data1 = np.asmatrix(data1)
        self._data2 = np.asmatrix(data2)
...
class DictLinearKernel():
    ...
    def computeKernelMatrix(self, data1, data2, symmetric=False):
        ...
        self.__km = None
        try:
            km = np.asmatrix(zeros((self._dim1, self._dim2), dtype=float64))
            if self._symmetric:
    ...
    def computeKernelMatrix(self, data1, data2, symmetric=False):
        ...
        try:
            if self._symmetric:
                ...
                trnorms = np.asmatrix(np.diag(linearkm)).T
                trace_matrix = trnorms * np.asmatrix(np.ones((1, self._dim1), dtype = float64))
                ...
                linkm = np.asmatrix(self._data1 * self._data2.T)
                trnorms1 = []

                for i in range(m):
                    trnorms1.append((self._data1[i] * self._data1[i].T)[0,0])
                trnorms1 = np.asmatrix(trnorms1).T
                trnorms2 = []
                for i in range(n):
                    trnorms2.append((self._data2[i] * self._data2[i].T)[0,0])
                trnorms2 = np.asmatrix(trnorms2).T
                self.__km = trnorms1 * np.asmatrix(np.ones((n, 1), dtype = float64)).T
                self.__km = self.__km + np.asmatrix(np.ones((m, 1), dtype = float64)) * trnorms2.T
